...
func NewInfiniBandCollector(logger log.Logger) (Collector, error) {
	...
	descriptions := map[string]string{
		...
		"excessive_buffer_overrun_errors":            "The number of times that OverrunErrors consecutive flow control update periods occurred, each having at least one overrun error.",
...
func (c *infinibandCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, device := range devices {
		...
		for _, port := range device.Ports {
			...
			c.pushCounter(ch, "excessive_buffer_overrun_errors", port.Counters.ExcessiveBufferOverrunErrors, port.Name, portStr)
			...
			c.pushCounter(ch, "local_link_integrity_errors", port.Counters.LocalLinkIntegrityErrors, port.Name, portStr)
